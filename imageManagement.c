...
unsigned char* littleEndianEncoding(int SIZE, int value) {
    unsigned char* bitsResult = calloc(SIZE, sizeof(unsigned char));
    if(bitsResult==NULL) {
        exit(0);
    }
    int i =0;
    ...
    for(int i =0; i<5; i++) {
        ...
        switch(i) {
            case 0: 
                ...
                break;
            case 1:
                result = littleEndianEncoding(size, image->header.size);
                break;
            case 2:
                result = littleEndianEncoding(size, image->header.createdBy);
                break;
            case 3: 
                result = littleEndianEncoding(size, image->header.createdByBis);
                break;
            case 4:
                result = littleEndianEncoding(size, image->header.offset);
                break;
        }
...
void encodeDIBHeader(FILE* file, Image* image) {
    ...
    for(int i =0; i<3; i++) {
        ...
        switch(i) {
            case 0:
                result = littleEndianEncoding(size, image->dibHeader.sizeHeader);
                break;
            case 1:
                result =  littleEndianEncoding(size, image->dibHeader.width);
                break;
            case 2: 
                result = littleEndianEncoding(size, image->dibHeader.height);
                break;
        }
